...
* @typedef {import('./types').DefineFastifyConfig} DefineFastifyConfig
*/

/** @returns {{ default: DefineFastifyRoutes, defineConfig?: DefineFastifyConfig } | undefined} */
// _astroFastifyEntry is injected via the vite plugin
// @ts-expect-error
const entrypointConfig = typeof _astroFastifyEntry != "undefined" ? _astroFastifyEntry : undefined;

const fastifyRoutes = entrypointConfig?.default ?? (() => {});
const defineFastifyConfig = entrypointConfig?.defineConfig ?? (() => {});
...
export function start(manifest, options) {
  ...
  const fastify = Fastify(defineFastifyConfig(config) ?? config);
