...
function vitePlugin(options) {
  return {
    ...
    async configureServer(server) {
      ...
      let setupRoutes = (server) => {};
      if(options?.entry) {
        const entry = entryToPath(options.entry);
        const entryModule = await server.ssrLoadModule(entry);
        setupRoutes = entryModule.default;
        if(typeof setupRoutes !== 'function') {
          throw new Error(`@matthewp/astro-fastify: ${entry} should export a default function.`);
        }

        if(entryModule.defineConfig) {
          defineConfig = entryModule.defineConfig;
          if(typeof defineConfig !== 'function') {
            throw new Error(`@matthewp/astro-fastify: ${entry} should export defineConfig as a function.`);
          }
        }
      }

      const fastify = Fastify(defineConfig(fastifyConfig) ?? fastifyConfig);
      setupRoutes(fastify);
    ...
    transform(code, id) {
      if(options?.entry && id.includes('@matthewp/astro-fastify/lib/server.js')) {
        ...
        let outCode = `import * as _astroFastifyEntry from "${entry}";\n${code}`;
        return outCode;
      }
    }
  }
